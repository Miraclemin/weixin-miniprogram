...
var food_set_ops = {
    ...
    eventBind: function () {
        ...
        $(".wrap_food_set .save").click(function () {
            ...
            var speaker_target = $(".wrap_food_set input[name=speaker]");
            var speaker = speaker_target.val();

            var summary = $.trim(that.ue.getContent());

            var speaker_address_target = $(".wrap_food_set input[name=speaker_address]");
            var speaker_address = speaker_address_target.val();


            var lecture_time_target = $(".wrap_food_set input[name=lecture_time]");
            var lecture_time = lecture_time_target.val();
            ...
            if (speaker.length < 1) {
                common_ops.tip("请输入符合规范的主讲人姓名~~", speaker_target);
                return;
            }
            ...
            if (summary.length < 10) {
                common_ops.tip("请输入描述，并不能少于10个字符~~", speaker_target);
                return;
            }

            if (speaker_address.length < 1) {
                common_ops.tip("请输入符合规范的地址~~", speaker_address_target);
                return;
            }
            if (lecture_time.length < 1) {
                common_ops.tip("请输入符合规范的讲座时间~~", lecture_time_target);
                return;
            }
            if (tags.length < 1) {
