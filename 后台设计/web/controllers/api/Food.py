...
def foodSearch():
    ...
    if mix_kw:
        rule = or_(Food.name.ilike("%{0}%".format(mix_kw)), Food.tags.ilike("%{0}%".format(mix_kw)),Food.speaker.ilike("%{0}%".format(mix_kw)),Food.speaker_address.ilike("%{0}%".format(mix_kw)),Food.lecture_time.ilike("%{0}%".format(mix_kw)))
    ...
    if food_list:
        for item in food_list:
            count_Cart = MemberCart.query.filter_by(food_id = item.id).count()
            
            tmp_data = {
                'id': item.id,
                'name': "%s"%( item.name ),
                'speaker':"%s"%(item.speaker) ,
                'speaker_address':"%s"%(item.speaker_address) ,
                'pic_url': UrlManager.buildImageUrl(item.main_image),
...
def foodInfo():
    ...
    resp['data']['info'] = {
        ...
        "speaker": food_info.speaker,
        "speaker_address":food_info.speaker_address,
        "lecture_time":food_info.lecture_time,
