...
def index():
    ...
    for item in list:
        list_return_temp={
        'count':PayOrderItem.query.filter_by( member_id = item.id,comment_status=1 ).count(),
        'item':item
        }
        list_return.append(list_return_temp)



    resp_data['list'] = list_return
...
def info():
    ...
    data_comment_list=[]
    if comment_list:
        comment_lecture_ids = selectFilterObj(comment_list,"food_ids")
        comment_lecture_map = getDictFilterField(Food,Food.id,"id",comment_lecture_ids)

        for item in comment_list:
            tmp_com_lec_info = comment_lecture_map[int(item.food_ids)]
            # print("####")
            # print(comment_lecture_map)
            data_comment_list.append({
                'name':tmp_com_lec_info.name,
                'score':item.score,
    ...
    #######计算学生完成讲座次数
    count=PayOrderItem.query.filter_by( member_id = id,comment_status=1 ).count()
    

    ######
    cat_mapping = getDictFilterField( FoodCat,FoodCat.id,"id",[] )
    lecture_list = PayOrderItem.query.filter_by(member_id=id,comment_status=1).all()
    data_lecture_list=[]
    if lecture_list:
        lecture_ids = selectFilterObj(lecture_list,"food_id")
        lecture_map = getDictFilterField(Food,Food.id,"id",lecture_ids)
        # lecture_info = {}
        for item in lecture_list:
            tmp_lec_info = lecture_map[item.food_id]
            data_lecture_list.append({
                'name':tmp_lec_info.name,
                'category':tmp_lec_info.cat_id,
    ...
    resp_data['info'] = info
    resp_data['data_lecture_list'] = data_lecture_list
    resp_data['comment_list'] = data_comment_list
    resp_data['current'] = 'index'
