...
def index():
    ...
    query = Food.query
    if 'mix_kw' in req:
        rule = or_(Food.name.ilike("%{0}%".format(req['mix_kw'])), Food.tags.ilike("%{0}%".format(req['mix_kw'])),Food.speaker.ilike("%{0}%".format(req['mix_kw'])),Food.speaker_address.ilike("%{0}%".format(req['mix_kw'])))
    ...
    for item in list:
        list_return_temp={
        'count': PayOrderItem.query.filter_by( food_id = item.id,comment_status=1 ).count(),
        'item':item
        }
...
def set():
    ...
    speaker = req['speaker'] if 'speaker' in req else ''
    main_image = req['main_image'] if 'main_image' in req else ''
    summary = req['summary'] if 'summary' in req else ''
    speaker_address = req['speaker_address'] if 'speaker_address' in req else ''
    tags = req['tags'] if 'tags' in req else ''
    lecture_time = req['lecture_time'] if 'lecture_time' in req else ''
    if cat_id < 1:
        resp['code'] = -1
        resp['msg'] = "请选择分类~~"
        return jsonify(resp)
    ...
    if name is None or len(name) < 1:
        resp['code'] = -1
        resp['msg'] = "请输入符合规范的讲座名称~~"
        return jsonify(resp)

    if not speaker or len( speaker ) < 1:
        resp['code'] = -1
        resp['msg'] = "请输入符合规范的主讲人姓名~~"
        return jsonify(resp)
...
#        return jsonify(resp)
    ...
    if summary is None or len(summary) < 1:
        resp['code'] = -1
        resp['msg'] = "请输入讲座描述，并不能少于10个字符~~"
        return jsonify(resp)

    if speaker_address is None or len(speaker_address) < 1:
        resp['code'] = -1
        resp['msg'] = "请输入符合规范的讲座地址~~"
        return jsonify(resp)
    if lecture_time is None or len(lecture_time) < 1:
        resp['code'] = -1
        resp['msg'] = "请输入符合规范的讲座时间~~"
        return jsonify(resp)
...
#        before_stock = model_food.stock
    ...
    model_food.speaker = speaker
    model_food.main_image = main_image
    model_food.summary = summary
    model_food.speaker_address = speaker_address
    model_food.lecture_time = lecture_time
